...
func TestDefineAndResolveBuiltIns(t *testing.T) {
	...
	for _, table := range []*SymbolTable{global, firstLocal, secondLocal} {
		for _, sym := range expected {
			...
			if !ok {
				t.Errorf("name %s did not resolve", sym.Name)
				continue
			}
			if result != sym {
				t.Errorf("expected %s to resolve to %+v, got=%+v", sym.Name, sym, result)
...
func TestResolveFree(t *testing.T) {
	...
	for _, test := range tests {
		...
		if len(test.table.FreeSymbols) != len(test.expectedFreeSymbols) {
			t.Errorf("wrong number of free symbols in table. expected=%d, got=%d", len(test.expectedFreeSymbols), len(test.table.FreeSymbols))
			continue
		}
		...
		for i, sym := range test.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("wrong free symbol. expected=%+v, got=%+v", sym, result)
			}
		}
	}
}
